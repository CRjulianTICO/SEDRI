...
DELIMITER $$
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_ActualizarEmpleado`(IN `CED` VARCHAR(20), IN `NOM` VARCHAR(35), IN `APE1` VARCHAR(35), IN `APE2` VARCHAR(35), IN `SEX` VARCHAR(20), IN `DIRECC` VARCHAR(50), IN `TEL` VARCHAR(25), IN `NAC` INT, IN `PUE` INT)
BEGIN
...
DELIMITER $$
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_InsertarEmpleado`(IN CED VARCHAR(20),IN NOM VARCHAR(35),IN APE1 VARCHAR(35),IN APE2 varchar(35),IN SEX VARCHAR(20),IN DIRECC VARCHAR(50), IN TEL VARCHAR(25),IN NAC INT,IN PUE INT)
BEGIN
